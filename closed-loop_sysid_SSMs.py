...
for epoch in range(epochs):
    ...
    y_hat_train_G, _ = Qg_SSM(input_data_training, state=None, mode="scan")

    # Calculate the mean squared error loss
    loss = MSE(y_hat_train_G, output_data_training)
...
if sys.vehicle:
    ...
    for i in range(2):
        ...
        plt.plot(time_plot, y_hat_train_S[i, 0:len(time_plot), 1].detach().numpy(), label="Modelled Output Y",
                 linestyle="--", color="red")
        plt.title(f"Real vs Modelled Outputs with an SSM model for S for Signal {i} in training set")
        plt.xlabel("Time (s)")
        plt.ylabel("Output")
        plt.legend()
        plt.tight_layout()
    ...
    for i in range(2):
        ...
        plt.plot(time_plot, y_hat_train_G[i, 0:len(time_plot), 0].detach().numpy(), label="Modelled Output X",
                 linestyle="--", color="orange")
        plt.title(f"Real vs Modelled Outputs with an SSM model for G for Signal {i} in training set")
        plt.xlabel("Time (s)")
        plt.ylabel("Output")
        plt.legend()
        plt.tight_layout()
